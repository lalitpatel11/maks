...
export default class SubFormHistoryBox extends Component {
                ...
                <View style={{ paddingVertical: hp(4) }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginBottom: hp(2) }}>
                        <View style={{ width: wp(40), borderWidth: 1, flexDirection: 'row', justifyContent: 'space-between', paddingVertical: hp(2), borderRadius: 50, paddingHorizontal: wp(3), alignItems: 'center', borderColor: '#8F8F8F' }}>
                            <Text style={{ fontSize: 15, color: '#8F8F8F' }}>{moment(item.filling_date).format("DD MMM YYYY")}</Text>
                            <Image
    ...
    <Text  style={{fontSize:15,color:'#8F8F8F',flex:1}}>{item.attachment_image == null ? 'Attach File' : this.fileNameFromUrl(item.attachment_image)}</Text>
   {
       ...
       :null
                    ...
                    <View style={{ flexDirection: 'row', alignSelf: 'center', justifyContent: 'space-between' }}>
                        ...
                        <TouchableOpacity onPress={() => { this.props.closeModal(), this.openUrl(item.preview) }}
                            style={{
                                width: wp(27), borderWidth: 1, flexDirection: 'row', justifyContent: 'space-between', paddingVertical: hp(1.5),
                        ...
                        </TouchableOpacity>
                        {
                            item.edit_from != ''
                                ...
                                <TouchableOpacity onPress={() => {
                                    this.props.closeModal(), this.props.navigation.navigate('FillFormDetails', {
                                        url: item.edit_from
                                    })
                                }}

                                    style={{
                                        width: wp(27), borderWidth: 1, flexDirection: 'row', justifyContent: 'space-between', paddingVertical: hp(1.5), borderRadius: 50,
